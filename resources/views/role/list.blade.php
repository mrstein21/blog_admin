            ...
            <div class="kt-portlet__body">
                <div class="tab-content">
                       @if (session('success'))
                        ...
                        @endif
                    @can("role-create")
                    <a href="{{route('role.view_add')}}"><button class="btn btn-primary">Tambah</button></a>
                    @endcan
                    <br>
                    <br>
...
$('#kt_1').DataTable({
    ...
    function showAlert(id){
      ...
      .then((willDelete) => {
        if (willDelete) {
          axios.delete("/role/delete/"+id).then((response)=>{
             swal("Data Role sudah dihapus!", {
             icon: "success",
            });
            //getData();
            location.reload();
