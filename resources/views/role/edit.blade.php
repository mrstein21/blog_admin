            ...
            <div class="kt-portlet__body">
                <div class="tab-content">
						...
						<div class="tab-pane active" id="kt_widget11_tab1_content">
                          <form class="kt-form" method="POST" action="{{ route('role.edit',$role->id) }}">
                            {{ csrf_field() }}
                            {{ method_field("PATCH")}}
							<div class="kt-portlet__body">
                                      ...
                                      <div class="col-lg-6">
                                        <select class="tags form-control" name="permission[]" multiple>
                                            @foreach($permission as $value)
                                                <option value="{{$value->id}}" @if(in_array($value->id,$role_permission)) selected @endif>{{$value->name}}</option>
                                            @endforeach
                                        </select>
                                       </div>
                                    </div>  
